...
RSpec.describe Movie do
  ...
  it "checks that the start_shooting method does 4 things by calling the mock Actor" do
    #write out assertions for each thing the start_shooting method would do
    expect(actor_double).to receive(:ready?)  #check that mock Actor (actor_double) received the call
    expect(actor_double).to receive(:act)
    expect(actor_double).to receive(:fall_off_cliff)
    expect(actor_double).to receive(:light_on_fire)
